...
test('history turns remains available as an agent history cap', () => {
  // The agent path uses the context window as a token budget and history_turns
  // as an additional turn cap; both constraints should remain editable.
  const guard = settingRowGuard('agent.editor.historyTurns')
  assert.match(guard, /v-if="formData\.config\.multi_turn_enabled \|\| isAgentMode"/)
})
