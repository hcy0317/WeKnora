    ...
    <t-form :inert="saving || undefined" ref="formRef" :data="formData" :rules="rules" layout="vertical">
      ...
      <section v-if="['embedding', 'chat', 'vllm'].includes(activeModelType) || formData.source === 'remote'" class="setting-drawer__section">
        ...
        <div v-if="showReasoningEffortField" class="form-item">
          <label class="form-label">{{ $t('model.editor.reasoningEffort') }}</label>
          <t-select :model-value="formData.extraConfig.reasoning_effort || ''" clearable
            @update:model-value="(value: string) => setExtraConfig('reasoning_effort', value)">
            <t-option v-for="option in reasoningEffortOptions" :key="option.value" :value="option.value"
              :label="option.label" />
          </t-select>
        ...
        <!--
          高级：协议覆盖 / 远端模型名 / 目录 compat 覆盖，以及可选的 thinking_control
          兼容覆盖。默认折叠，绝大多数用户无需触碰。
        -->
...
interface ModelFormData {
  ...
  /**
   * extra_config entries: vendor-declared fields plus connection-wide
   * overrides such as `api`, `remote_model_name`, and `reasoning_effort`.
   * thinking_control lives in its own field above.
   */
...
const visibleExtraFields = computed<ModelProviderExtraField[]>(() =>
  extraFieldsForModelType(selectedProvider.value?.extraFields, activeModelType.value)
    .filter(field => !RESERVED_EXTRA_CONFIG_KEYS.has(field.key)
      && !(field.key === 'reasoning_effort' && showReasoningEffortField.value)),
)
/**
...
const VENDOR_NEUTRAL_EXTRA_CONFIG_KEYS = ['api', 'remote_model_name', 'reasoning_effort'] as const
...
const reasoningEffortOptions = computed(() => {
  const catalogLevels = resolvedThinkingLevels.value.filter(level => level !== 'off' && level !== 'auto')
  const levels = catalogLevels.length > 0
    ? catalogLevels
    : ['minimal', 'low', 'medium', 'high', 'xhigh', 'max']
  return levels.map(value => ({ value, label: t(levelLabelKey(value)) }))
})
...
const checkRemoteAPI = async () => {
  ...
  try {
    ...
    switch (activeModelType.value) {
      ...
      case 'vllm':
        // VLLM 模型（多模态）
        // 按 VLLM 类型测试多模态协议，并携带模型级 reasoning 配置。
        result = await checkRemoteModel({
          modelType: 'vllm',
          modelName: formData.value.modelName,
          baseUrl: formData.value.baseUrl || '',
