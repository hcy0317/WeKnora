export default {
  ...
  model: {
    ...
    editor: {
      ...
      reasoningEffortDesc: '호환 API에 reasoning_effort를 항상 전송합니다. 공급자 기본값을 사용하려면 비워 두세요.',
  ...
  graphSettings: {
    ...
    configIncomplete: '지식 그래프 추출이 활성화되어 있습니다. 저장하기 전에 예시 텍스트, 관계 유형, 엔터티 및 관계를 모두 입력하세요',
  ...
  settings: {
    ...
    engineLifecycle: {
      title: '실행 수명 주기',
      description: '작업이 들어오면 엔진을 시작하고 완전히 유휴 상태가 되면 자동으로 중지합니다.',
      adminOnly: '시스템 관리자만 상태를 보고 수명 주기 정책을 변경할 수 있습니다.',
      controllerOffline: '호스트 컨트롤러를 사용할 수 없습니다. 마지막 구성은 유지되지만 저장할 수 없습니다.',
      observeOnly: '관찰 전용 모드입니다. 정책은 구성할 수 있지만 엔진을 자동으로 시작하거나 중지하지 않습니다.',
      mode: '실행 모드',
      onDemand: '필요할 때 시작',
      onDemandHint: '첫 요청은 준비 완료를 기다리고, 완전 유휴 시간이 임계값에 도달하면 중지됩니다.',
      alwaysOn: '항상 실행',
      alwaysOnHint: '컨트롤러가 이 엔진 그룹을 계속 사용 가능한 상태로 유지합니다.',
      idleOverride: '이 그룹의 유휴 시간 재정의(분)',
      defaultIdle: '재정의하지 않으면 전역 기본값 {minutes}분을 사용합니다.',
      startupOverride: '이 그룹의 시작 대기 시간 재정의(초)',
      defaultStartup: '재정의하지 않으면 전역 기본값 {seconds}초를 사용합니다.',
      apply: '정책 적용',
      refresh: '새로 고침',
      saved: '수명 주기 정책을 저장했습니다.',
      saveFailed: '수명 주기 정책을 저장하지 못했습니다.',
      conflict: '다른 관리자가 구성을 업데이트했습니다(최신 리비전 {revision}). 새로 고친 뒤 입력을 검토하고 다시 적용하세요.',
      conflictRefreshed: '최신 리비전을 불러왔고 입력은 유지했습니다. 검토 후 다시 적용하세요.',
      revision: '리비전',
      active: '활성 요청',
      pending: '대기 요청',
      gpuAdmission: 'GPU 허용',
  ...
  knowledgeStages: {
    ...
    retry: '다시 파싱',
    retryItem: '이 항목 재시도',
    retryItemConfirm: '새 부분 복구 기록을 만들고 “{name}” 항목만 다시 실행할까요? 완료된 항목은 다시 실행되지 않지만 모델 할당량이 사용될 수 있습니다.',
    retryItemSubmitted: '항목 재시도가 제출되었습니다',
    retryItemFailed: '항목 재시도 제출에 실패했습니다',
    retryFailedItems: '실패 항목 필요 시 재시도',
    retryFailedCounts: 'Wiki {wiki} · 요약 {summary} · 그래프 {graph} · 질문 {question}',
    retryFailedItemsConfirm: '새 부분 복구 기록을 만들고 다음 {count}개 대상만 다시 실행합니다: {targets}. 대상 통계: {counts}. 다른 항목은 다시 실행되지 않지만 모델 할당량이 사용될 수 있습니다. 계속할까요?',
    retryFailedItemsSubmitted: '필요 시 재시도가 제출되어 새 부분 복구 시도를 여는 중입니다',
    retryFailedItemsFailed: '필요 시 재시도 제출에 실패했습니다',
    retryError: {
      forbidden: '재시도 권한이 없습니다. 지식 베이스 관리자에게 Editor/Admin 권한을 확인해 달라고 요청하세요.',
      conflict: '상태가 변경되었거나 복구 시도가 이미 있습니다. 최신 시도를 새로고침했으니 확인 후 다시 시도하세요.',
      unavailable: '재시도 작업을 게시하지 못했습니다. 처리 상태를 새로고침했으니 잠시 후 다시 제출하세요.',
      generic: '재시도 제출에 실패했습니다. 새로고침 후 다시 시도하세요.',
    },
    retryReason: {
      unavailable: '서버가 재시도 가능한 대상을 제공하지 않았습니다. 새로고침 후 다시 시도하세요.',
      no_retryable_targets: '서버가 허용한 실패 또는 중단 항목이 없습니다.',
      historical_attempt: '과거 시도는 읽기 전용입니다. 최신 시도로 전환하세요.',
      active_owner: '대상 작업자가 아직 활성 상태이므로 중복 제출할 수 없습니다.',
      repair_in_progress: '부분 복구가 이미 진행 중입니다. 최신 시도를 먼저 확인하세요.',
      liveness_read_failed: '서버가 작업 생존 상태를 확인하지 못해 재시도를 안전하게 비활성화했습니다.',
      liveness_unavailable: '서버가 작업 생존 상태를 확인하지 못해 재시도를 안전하게 비활성화했습니다.',
    },
    refresh: '지금 새로고침',
    copy: '복사',
    copyDetails: '세부정보 복사',
    ...
    transport: {
      title: '모델 스트림 전송',
      inProgress: '요청이 진행 중입니다. 토큰 사용량은 완전한 종료 이벤트 후에만 표시됩니다.',
      waitingForCompletion: '스트림 이벤트를 받았으며 response.completed를 기다리는 중입니다. 완료 전에는 내용을 저장하지 않습니다.',
      completed: '완전한 종료 이벤트를 받아 출력과 사용량을 정산했습니다.',
      failed: '완료 전에 스트림이 실패하여 불완전한 내용을 성공 출력으로 저장하지 않았습니다.',
      cancelled: '요청이 취소되어 불완전한 내용을 성공 출력으로 저장하지 않았습니다.',
      unavailableUsageShell: '이 이전 값은 사용량을 알 수 없다는 자리표시자이며 모델 출력이나 성공 증거가 아닙니다. 원본은 원시 JSON에 유지됩니다.',
      protocol: '프로토콜',
      endpoint: '엔드포인트',
      firstEvent: '첫 SSE 이벤트',
      step: {
        created: '생성됨',
        request_dispatched: '요청 전송',
        stream_opened: '스트림 연결',
        first_sse_event: '이벤트 수신',
        completed: '완료'
      }
    },
    token: {
      title: '모델 토큰 사용량',
      hint: '이 문서의 모델 호출을 처리 단계와 모델별로 집계합니다. 프롬프트와 전체 출력은 Langfuse에 보관됩니다.',
      input: '입력',
      output: '출력',
      cacheRead: '캐시 읽기',
      cacheWrite: '캐시 쓰기',
      total: '합계',
      tokens: '토큰',
      calls: '총 {calls}회 호출(측정 {measured} / 추정 {estimated} / 알 수 없음 {unknown})',
      callsShort: '호출',
      model: '모델',
      empty: '이 문서에는 아직 집계할 모델 토큰 데이터가 없습니다.',
      stage: {
        summary: '요약 생성',
        question: 'AI 질문 생성',
        graph: '지식 그래프 추출',
