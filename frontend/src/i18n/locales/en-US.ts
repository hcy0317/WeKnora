export default {
  ...
  knowledgeStages: {
    ...
    transport: {
      title: 'Model stream transport',
      inProgress: 'The request is still running. Token usage is available only after a complete terminal event.',
      waitingForCompletion: 'Stream events are arriving; waiting for response.completed. Content is not persisted before completion.',
      completed: 'A complete terminal event was received and the output and usage were settled.',
      failed: 'The stream failed before completion; partial content was not recorded as a successful output.',
      cancelled: 'The request was cancelled; partial content was not recorded as a successful output.',
      unavailableUsageShell: 'This legacy value is an unavailable-usage placeholder, not model output or proof of success. The raw value remains in Raw JSON.',
      protocol: 'Protocol',
      endpoint: 'Endpoint',
      firstEvent: 'First SSE event',
      step: {
        created: 'Created',
        request_dispatched: 'Request sent',
        stream_opened: 'Stream opened',
        first_sse_event: 'Event received',
        completed: 'Completed'
      }
    },
    token: {
      title: 'Model token usage',
      hint: 'Aggregated by processing stage and model for this document. Prompts and complete outputs remain in Langfuse.',
      input: 'Input',
      output: 'Output',
      ...
      total: 'Total',
      tokens: 'tokens',
      calls: '{calls} calls (measured {measured} / estimated {estimated} / unknown {unknown})',
      callsShort: 'Calls',
      model: 'Model',
      empty: 'No model token usage is available for this document yet.',
      stage: {
        summary: 'Summary generation',
        question: 'AI question generation',
        graph: 'Knowledge graph extraction',
        wiki: 'Wiki generation',
        datatable: 'Data table summary'
      }
    },
    attempt: 'Attempt {n}',
    retry: 'Retry parsing',
    retryItem: 'Retry item',
    retryItemConfirm: 'Create a new partial-repair history and retry only “{name}”? Completed items will not run again, but model quota may be consumed.',
    retryItemSubmitted: 'Item retry submitted',
    retryItemFailed: 'Failed to submit item retry',
    retryFailedItems: 'Retry failed items',
    retryFailedCounts: 'Wiki {wiki} · Summary {summary} · Graph {graph} · Questions {question}',
    retryFailedItemsConfirm: 'Create a new partial-repair history and retry only these {count} targets: {targets}. Counts: {counts}. Other items will not run again, but model quota may be consumed.',
    retryFailedItemsSubmitted: 'On-demand retry submitted; opening the new partial-repair attempt',
    retryFailedItemsFailed: 'Failed to submit on-demand retry',
    retryError: {
      forbidden: 'You cannot retry this item. Ask a knowledge-base administrator to confirm Editor/Admin access.',
      conflict: 'The state changed or a repair already exists. The latest attempt has been refreshed; review it before retrying.',
      unavailable: 'The retry task could not be published. Processing state was refreshed; try again later.',
      generic: 'Retry submission failed. Refresh and try again.',
    },
    retryReason: {
      unavailable: 'The server has not provided retryable targets. Refresh and try again.',
      no_retryable_targets: 'There are no server-authorized failed or stalled items.',
      historical_attempt: 'Historical attempts are read-only. Switch to the latest attempt.',
      active_owner: 'This target still has an active worker and cannot be submitted again.',
      repair_in_progress: 'A partial repair is already running. Review the latest attempt first.',
      liveness_read_failed: 'The server could not verify task liveness, so retry is safely disabled.',
      liveness_unavailable: 'The server could not verify task liveness, so retry is safely disabled.',
    },
    refresh: 'Refresh now',
    copy: 'Copy',
  ...
  settings: {
    ...
    engineLifecycle: {
      ...
      description: 'Start the engine when work arrives and stop it after it is fully idle.',
      adminOnly: 'Only system administrators can view status or change lifecycle policy.',
      controllerOffline: 'The host controller is unavailable. The last configuration is retained, but saving is disabled.',
      observeOnly: 'Observe-only mode is active: policies can be configured, but engines are not started or stopped automatically.',
      mode: 'Runtime mode',
      onDemand: 'On demand',
      onDemandHint: 'The first request waits for readiness; the engine stops after the full idle threshold.',
      alwaysOn: 'Always on',
      alwaysOnHint: 'The controller keeps this engine group available and skips idle shutdown.',
      idleOverride: 'Override idle time for this group (minutes)',
      defaultIdle: 'Uses the global default when not overridden: {minutes} minutes.',
      startupOverride: 'Override startup wait for this group (seconds)',
      defaultStartup: 'Uses the global default when not overridden: {seconds} seconds.',
      apply: 'Apply policy',
      refresh: 'Refresh',
      saved: 'Lifecycle policy saved',
      saveFailed: 'Failed to save lifecycle policy',
      conflict: 'Another administrator updated the configuration (latest revision {revision}). Refresh, review, and reapply your input.',
      conflictRefreshed: 'The latest revision is loaded and your input was preserved. Review and apply it again.',
      revision: 'Revision',
    ...
    parser: {
      ...
      standby: 'Standby · starts when needed',
      starting: 'Starting',
  ...
  graphSettings: {
    ...
    configIncomplete: 'Knowledge graph extraction is enabled. Complete the sample text, relationship types, entities, and relationships before saving',
  ...
  model: {
    ...
    editor: {
      ...
      reasoningEffortDesc: 'Always sends reasoning_effort to compatible APIs. Leave empty to use the provider default; set it only when the provider documents support.',
      thinkingControl: {
        none: {
