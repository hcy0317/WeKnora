export default {
  ...
  model: {
    ...
    editor: {
      ...
      reasoningEffortDesc: '固定向兼容接口发送 reasoning_effort。留空表示使用上游默认；仅在服务商明确支持时设置。',
  ...
  graphSettings: {
    ...
    configIncomplete: '知识图谱已启用，请完整填写示例文本、关系类型、实体和关系后再保存',
  ...
  settings: {
    ...
    engineLifecycle: {
      title: '运行生命周期',
      description: '按任务需要启动引擎，并在完全空闲后自动停止。',
      adminOnly: '仅系统管理员可以查看状态和修改生命周期策略。',
      controllerOffline: '宿主控制器当前不可用。已保留上次读取的配置，但暂时不能保存。',
      observeOnly: '当前为仅观察模式：可以配置策略，但不会自动启停引擎。',
      mode: '运行模式',
      onDemand: '按需启动',
      onDemandHint: '首个请求会等待引擎就绪；完全空闲达到阈值后自动停止。',
      alwaysOn: '始终运行',
      alwaysOnHint: '控制器会保持该组引擎可用，不执行空闲停止。',
      idleOverride: '覆盖本组空闲时间（分钟）',
      defaultIdle: '未覆盖时使用全局默认值：{minutes} 分钟。',
      startupOverride: '覆盖本组启动等待时间（秒）',
      defaultStartup: '未覆盖时使用全局默认值：{seconds} 秒。',
      apply: '应用策略',
      refresh: '刷新',
      saved: '生命周期策略已保存',
      saveFailed: '生命周期策略保存失败',
      conflict: '配置已被其他管理员更新（最新修订 {revision}）。请刷新后检查并重新应用当前输入。',
      conflictRefreshed: '已载入最新修订并保留你的输入；请检查后重新应用。',
      revision: '修订',
      active: '活动请求',
      pending: '等待请求',
      gpuAdmission: 'GPU 准入',
  ...
  knowledgeStages: {
    ...
    retry: '重新解析',
    retryItem: '重试此项',
    retryItemConfirm: '将新建一轮局部修复历史，仅重试“{name}”；其他已完成项目不会重跑，但可能消耗模型额度。继续吗？',
    retryItemSubmitted: '单项重试已提交',
    retryItemFailed: '单项重试提交失败',
    retryFailedItems: '按需重试失败项',
    retryFailedCounts: 'Wiki {wiki} · 摘要 {summary} · 图谱 {graph} · 问题 {question}',
    retryFailedItemsConfirm: '将新建一轮局部修复历史，仅重试以下 {count} 项：{targets}。目标统计：{counts}。其他项目不会重跑，但可能消耗模型额度。继续吗？',
    retryFailedItemsSubmitted: '按需重试已提交，正在打开新的局部修复轮次',
    retryFailedItemsFailed: '按需重试提交失败',
    retryError: {
      forbidden: '你没有重试权限，请联系知识库管理员确认 Editor/Admin 权限。',
      conflict: '状态已变化或已有局部修复轮次；已刷新到最新状态，请检查后再重试。',
      unavailable: '重试任务发布失败；已刷新处理状态，请稍后再次提交。',
      generic: '重试提交失败，请刷新后再试。',
    },
    retryReason: {
      unavailable: '服务端暂未提供可重试目标，请刷新后再试。',
      no_retryable_targets: '当前没有服务端授权的失败或卡住项目。',
      historical_attempt: '历史轮次只读，请切换到最新轮次。',
      active_owner: '目标仍有真实执行者，暂不能重复提交。',
      repair_in_progress: '已有局部修复轮次正在处理，请先查看最新轮次。',
      liveness_read_failed: '服务端暂时无法确认任务存活状态，已安全禁用重试。',
      liveness_unavailable: '服务端暂时无法确认任务存活状态，已安全禁用重试。',
    },
    refresh: '立即刷新',
    copy: '复制',
    copyDetails: '复制详情',
    ...
    transport: {
      title: '模型流传输',
      inProgress: '请求仍在进行中，Token 用量仅在收到完整结束事件后可用。',
      waitingForCompletion: '已收到流事件，正在等待 response.completed；结束前不会把内容落库。',
      completed: '已收到完整结束事件，模型输出与用量已结算。',
      failed: '流在完整结束前失败，未把不完整内容作为成功输出。',
      cancelled: '请求已取消，未把不完整内容作为成功输出。',
      unavailableUsageShell: '这是旧记录中的不可用用量占位，不是模型输出，也不能据此判断请求成功。原始值仍保留在「原始 JSON」。',
      protocol: '协议',
      endpoint: '端点',
      firstEvent: '首个 SSE 事件',
      step: {
        created: '已创建',
        request_dispatched: '已发出请求',
        stream_opened: '已建立流',
        first_sse_event: '已收到事件',
        completed: '完整结束'
      }
    },
    token: {
      title: '模型 Token 用量',
      hint: '按处理阶段和模型汇总当前文档的模型调用；提示词和完整输出仍保存在 Langfuse。',
      input: '输入',
      output: '输出',
      cacheRead: '缓存读取',
      cacheWrite: '缓存写入',
      total: '总计',
      tokens: 'Token',
      calls: '共 {calls} 次调用（实测 {measured} / 估算 {estimated} / 未知 {unknown}）',
      callsShort: '调用',
      model: '模型',
      empty: '此文档暂时没有可汇总的模型 Token 数据。',
      stage: {
        summary: '摘要生成',
        question: 'AI 问题生成',
        graph: '知识图谱提取',
