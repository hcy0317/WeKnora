export default {
  ...
  model: {
    ...
    editor: {
      ...
      reasoningEffortLabel: 'Уровень рассуждения',
      reasoningEffortDesc: 'Всегда отправляет reasoning_effort совместимому API. Оставьте пустым, чтобы использовать значение поставщика по умолчанию.',
  ...
  graphSettings: {
    ...
    configIncomplete: 'Извлечение графа знаний включено. Перед сохранением заполните пример текста, типы связей, сущности и связи',
  ...
  settings: {
    ...
    engineLifecycle: {
      title: 'Жизненный цикл среды',
      description: 'Запускает движок при поступлении задачи и останавливает после полного простоя.',
      adminOnly: 'Только системные администраторы могут просматривать состояние и менять политику.',
      controllerOffline: 'Контроллер узла недоступен. Последняя конфигурация сохранена, но запись отключена.',
      observeOnly: 'Включён режим наблюдения: политику можно настроить, но движки не запускаются и не останавливаются автоматически.',
      mode: 'Режим работы',
      onDemand: 'По требованию',
      onDemandHint: 'Первый запрос ждёт готовности; после полного простоя движок автоматически останавливается.',
      alwaysOn: 'Всегда включён',
      alwaysOnHint: 'Контроллер поддерживает доступность этой группы и не останавливает её при простое.',
      idleOverride: 'Переопределить время простоя группы (минуты)',
      defaultIdle: 'Без переопределения используется глобальное значение: {minutes} мин.',
      startupOverride: 'Переопределить ожидание запуска группы (секунды)',
      defaultStartup: 'Без переопределения используется глобальное значение: {seconds} с.',
      apply: 'Применить политику',
      refresh: 'Обновить',
      saved: 'Политика жизненного цикла сохранена',
      saveFailed: 'Не удалось сохранить политику жизненного цикла',
      conflict: 'Другой администратор обновил конфигурацию (последняя редакция {revision}). Обновите данные, проверьте и примените ввод повторно.',
      conflictRefreshed: 'Последняя редакция загружена, ваш ввод сохранён. Проверьте и примените его повторно.',
      revision: 'Редакция',
    ...
    parser: {
      ...
      standby: 'Ожидание · запуск по запросу',
  ...
  knowledgeStages: {
    ...
    retryItem: 'Повторить элемент',
    retryItemConfirm: 'Создать новую историю частичного восстановления и повторить только «{name}»? Завершённые элементы не запустятся снова, но может расходоваться квота модели.',
    retryItemSubmitted: 'Повтор элемента запущен',
    retryItemFailed: 'Не удалось запустить повтор элемента',
    retryFailedItems: 'Повторить сбойные элементы',
    retryFailedCounts: 'Wiki {wiki} · Сводка {summary} · Граф {graph} · Вопросы {question}',
    retryFailedItemsConfirm: 'Создать новую историю частичного восстановления и повторить только эти цели ({count}): {targets}. Состав: {counts}. Остальные элементы не запустятся снова, но может расходоваться квота модели. Продолжить?',
    retryFailedItemsSubmitted: 'Повтор по требованию отправлен; открывается новая попытка частичного восстановления',
    retryFailedItemsFailed: 'Не удалось отправить повтор по требованию',
    retryError: {
      forbidden: 'У вас нет права на повтор. Попросите администратора базы знаний проверить роль Editor/Admin.',
      conflict: 'Состояние изменилось или восстановление уже создано. Последняя попытка обновлена; проверьте её перед повтором.',
      unavailable: 'Не удалось опубликовать задачу повтора. Состояние обработки обновлено; повторите позже.',
      generic: 'Не удалось отправить повтор. Обновите страницу и попробуйте снова.',
    },
    retryReason: {
      unavailable: 'Сервер не предоставил доступные для повтора цели. Обновите данные и попробуйте снова.',
      no_retryable_targets: 'Нет разрешённых сервером сбойных или зависших элементов.',
      historical_attempt: 'Исторические попытки доступны только для чтения. Перейдите к последней попытке.',
      active_owner: 'У цели всё ещё есть активный исполнитель, поэтому повторная отправка недоступна.',
      repair_in_progress: 'Частичное восстановление уже выполняется. Сначала проверьте последнюю попытку.',
      liveness_read_failed: 'Сервер не смог проверить активность задачи, поэтому повтор безопасно отключён.',
      liveness_unavailable: 'Сервер не смог проверить активность задачи, поэтому повтор безопасно отключён.',
    },
    refresh: 'Refresh now',
    copy: 'Copy',
    ...
    transport: {
      title: 'Поток модели',
      inProgress: 'Запрос выполняется. Токены доступны только после полного события завершения.',
      waitingForCompletion: 'События потока поступают; ожидается response.completed. До завершения содержимое не сохраняется.',
      completed: 'Получено полное событие завершения; ответ и токены сохранены.',
      failed: 'Поток завершился ошибкой; частичный ответ не отмечен как успешный.',
      cancelled: 'Запрос отменен; частичный ответ не отмечен как успешный.',
      unavailableUsageShell: 'Это старый заполнитель недоступной статистики, а не ответ модели или признак успеха. Исходное значение остается в Raw JSON.',
      protocol: 'Протокол',
      endpoint: 'Эндпоинт',
      firstEvent: 'Первое событие SSE',
      step: {
        created: 'Создан',
        request_dispatched: 'Запрос отправлен',
        stream_opened: 'Поток открыт',
        first_sse_event: 'Событие получено',
        completed: 'Завершен'
      }
    },
    token: {
      title: 'Использование токенов модели',
      hint: 'Вызовы модели для этого документа сгруппированы по этапу обработки и модели. Промпты и полные ответы остаются в Langfuse.',
      input: 'Ввод',
      output: 'Вывод',
      ...
      total: 'Всего',
      tokens: 'токенов',
      calls: 'Вызовов: {calls} (измерено {measured} / оценено {estimated} / неизвестно {unknown})',
      callsShort: 'Вызовы',
      model: 'Модель',
      empty: 'Для этого документа пока нет данных об использовании токенов модели.',
      stage: {
        summary: 'Создание резюме',
        question: 'Генерация вопросов ИИ',
        graph: 'Извлечение графа знаний',
        wiki: 'Создание Wiki',
        datatable: 'Резюме таблицы данных'
      }
    }
  },
  uploadConfirm: {
