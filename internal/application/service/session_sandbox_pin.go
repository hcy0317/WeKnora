...
func resolveSandboxForExecution(
	...
	if !hasNamedSandboxConfig(configID) {
		...
		// Preserve the local default-manager route when a deployment has not
		// configured a named backend. Only named backends receive a session pin.
		mgr, err := resolveTenantSandboxForConfig(ctx, resolver, fallback, tenantID, configID, policy)
		return mgr, SandboxPin{}, err
	}
