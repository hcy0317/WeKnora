...
// A processing document whose only outstanding work is a queued Wiki ingest
// is invisible to the per-knowledge asynq probe. The durable pending-op row
// must therefore keep housekeeping from treating the document as orphaned.
func TestHousekeeping_NoFalseKill_DurableWikiIngestPending(t *testing.T) {
	db := setupHousekeepingDB(t)
	svc := newHousekeepingSvcWithInspector(db, fakeTaskInspector{})
	stale := time.Now().Add(-3 * time.Hour)
	insertKnowledge(t, db, "kid-durable-wiki", types.ParseStatusProcessing, stale)
...
func TestHousekeeping_StillRecoversWhenNoDurableOp(t *testing.T) {
	...
	svc := newHousekeepingSvcWithInspector(db, fakeTaskInspector{})
	stale := time.Now().Add(-3 * time.Hour)
	insertKnowledge(t, db, "kid-orphan", types.ParseStatusProcessing, stale)
