...
func TestSharedExecutionFiltersDocumentsChunksAndSearchScope(t *testing.T) {
	...
	for _, execution := range []uint64{1, 2, 3} {
		...
		require.ElementsMatch(t, []string{"own", "granted"}, got)
		require.NoError(t, search.authorizeKBAccess(ctx, kbs[:2], execution))
		require.Error(t, search.authorizeKBAccess(ctx, kbs[2:3], execution), "execution tenant is not ownership")
		require.Error(t, search.authorizeKBAccess(ctx, kbs[3:], execution), "source tenant shares are not inherited")
...
func TestSharedAgentBatchUsesGrantInsteadOfSourceTenantOwnership(t *testing.T) {
	...
	app, ok := apperrors.IsAppError(
		search.authorizeKBAccess(ctx, []*types.KnowledgeBase{{ID: "selected", TenantID: 2}}, 2),
	)
	require.True(t, ok)
