...
// work is a queued Wiki ingest. Probe the durable table directly before
// consulting the inspector.
//
// When no inspector is wired (nil) the durable gate still applies; only the
// transient queue check is skipped. Either probe failing defers the affected
// candidate to a later sweep because ownership is unknown.
