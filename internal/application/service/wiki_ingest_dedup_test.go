...
func TestRemapSlugUpdatesByIdentityConverges(t *testing.T) {
	...
	got, aliases := svc.remapSlugUpdatesByIdentity(ctx, "kb-1", map[string][]SlugUpdate{
		"entity/kong-zi": {{
	...
	if aliases["entity/confucius"] != "entity/kong-zi" {
		t.Fatalf("identity remap did not report the doc-result alias: %#v", aliases)
	}
	results := []*docIngestResult{{
		Summary: "See [[entity/confucius]]",
		Pages:   []wikiIngestPageRef{{Slug: "entity/confucius", Title: "孔子"}},
	}}
	rewriteDocResultSlugs(results, aliases)
	if results[0].Summary != "See [[entity/kong-zi]]" || len(results[0].Pages) != 1 ||
		results[0].Pages[0].Slug != "entity/kong-zi" {
		t.Fatalf("identity remap left doc result bookkeeping stale: %#v", results[0])
	}
}
...
func (s *stubNormalizedTitleWiki) FindSimilarPages(
...
func TestDeduplicateExtractedBatchKeepsDeterministicCrossTypeFallback(t *testing.T) {
	stub := &stubNormalizedTitleWiki{pages: []*types.WikiPageLite{{
		Slug: "concept/yumi-wen-ku-bing", Title: "玉米纹枯病", PageType: types.WikiPageTypeConcept,
	}}}
	svc := &wikiIngestService{wikiService: stub}
	entities, concepts := svc.deduplicateExtractedBatch(
		context.Background(), nil, "kb-1",
		[]extractedItem{{Name: "玉米纹枯病", Slug: "entity/yumi-wenku-bing"}}, nil,
		&WikiBatchContext{},
	)
	if len(entities) != 1 || entities[0].Slug != "concept/yumi-wen-ku-bing" || len(concepts) != 0 {
