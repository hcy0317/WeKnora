	...
	*diag = res.Diagnostics
	...
	if diag.Outcome == types.RerankOutcomeModelError {
		if rerank.IsLifecycleManaged(rerankModel) {
			modelErr := fmt.Errorf("%s", diag.Error)
			rerankSpan.Finish(map[string]interface{}{
				"stage": "managed_api_error", "candidate_count": diag.CandidateCount, "error": diag.Error,
			}, nil, modelErr)
			return ErrRerank.WithError(modelErr)
		}
		// External model failures keep the retrieval fallback; managed failures above
		// remain visible because backend failover belongs to the engine lifecycle.
