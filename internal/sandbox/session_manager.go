...
type SessionBoundManager struct {
	...
	// legacyTurnMu pairs the old Begin/End API with exact token releases.
	// New callers should use HoldSessionTurn, which carries the release token
	// directly and cannot release a later turn after an ABA transition.
	legacyTurnMu       sync.Mutex
	legacyTurnReleases map[string][]func() error
}
...
type ShellExecOptions struct {
	...
	// Root is only selected when the trusted installer explicitly asks for it;
	// the bootstrap still differs from ordinary workspace preparation.
...
// BeginSessionTurn preserves the pre-token API for callers that have not yet
// moved to HoldSessionTurn. Its matching EndSessionTurn releases the exact
// token created here.
func (m *SessionBoundManager) BeginSessionTurn(ctx context.Context, sessionID string) error {
	if m == nil {
		return nil
	}
	release, err := m.HoldSessionTurn(ctx, sessionID)
	if err != nil {
		return err
	}
	m.legacyTurnMu.Lock()
	if m.legacyTurnReleases == nil {
		m.legacyTurnReleases = make(map[string][]func() error)
	}
	m.legacyTurnReleases[sessionID] = append(m.legacyTurnReleases[sessionID], release)
	m.legacyTurnMu.Unlock()
	return nil
}
...
func (m *SessionBoundManager) EndSessionTurn(ctx context.Context, sessionID string) error {
	...
	m.legacyTurnMu.Lock()
	stack := m.legacyTurnReleases[sessionID]
	if len(stack) == 0 {
		m.legacyTurnMu.Unlock()
		return nil
	}
	release := stack[len(stack)-1]
	if len(stack) == 1 {
		delete(m.legacyTurnReleases, sessionID)
	} else {
		m.legacyTurnReleases[sessionID] = stack[:len(stack)-1]
	}
	m.legacyTurnMu.Unlock()
	return release()
}
...
// Ordinary calls can use the workspace and temporary directories; installer
// calls also get the mounted skills root. System paths remain outside the API
// surface, and the command still runs as the unprivileged sandbox account.
func cleanSessionWorkDir(workDir string, allowSkillsRoot bool) (string, error) {
	...
	allowedRoots := []string{SessionWorkspaceRoot, "/tmp", "/var/tmp"}
	if allowSkillsRoot {
		allowedRoots = append(allowedRoots, SkillsImageRoot)
	}
	...
	return "", fmt.Errorf(
		...
		workDir, strings.Join(allowedRoots, ", "),
	)
}
