...
type RemoteExecRequest struct {
	...
	// CommandOptions.User).
	//
	// The default is the unprivileged sandbox account. Root is available only
	// through explicit maintenance operations; ordinary model-authored work
	// must retain the container's filesystem permission boundary.
	User string
...
// DefaultSandboxExecUser is the unprivileged account used for sandboxed work.
// Root remains selectable for explicit maintenance operations, while ordinary
// model-authored commands and file operations stay inside the sandbox account's
