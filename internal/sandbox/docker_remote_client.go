...
//
// Every file operation uses exec with an explicit account (the unprivileged
// sandbox user by default; trusted install paths may explicitly use root),
// timeout and activity tracking. Archive endpoints bypass those exec settings.
// File-path containment still depends on the selected account and its mounts:
...
//
// A blank user resolves to DefaultSandboxExecUser, the unprivileged account: this
...
//
// Root must be selected explicitly for trusted maintenance operations.
...
//
// Ordinary operations use the unprivileged sandbox account, so the kernel
// enforces its ownership and mode checks. Trusted installer operations may
// explicitly select root and must keep their paths inside the managed skills
// tree; lexical path guards are not a substitute for that ownership boundary.
