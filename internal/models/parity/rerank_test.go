...
// TestRerankCeilingsAreTheEffectiveOnes pins the limits the shared batching
// layer enforces. It uses a client-side limit when the pinned SDK is stricter
// than the public service documentation.
func TestRerankCeilingsAreTheEffectiveOnes(t *testing.T) {
	for id, want := range map[string]api.RerankSettings{
		...
		// The service documents 200, but the pinned vikingdb-go-sdk rejects
		// more than 50 datas locally before sending the request.
