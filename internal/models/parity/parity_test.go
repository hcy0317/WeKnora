...
func TestMain(m *testing.M) {
	hosts := map[string]struct{}{"127.0.0.1": {}}
	for _, host := range []string{
		"generativelanguage.googleapis.com", "router.requesty.ai", "api.novita.ai",
		"integrate.api.nvidia.com",
	} {
		hosts[host] = struct{}{}
	}
	for _, definition := range providers.Builtins() {
		for _, baseURL := range definition.DefaultBaseURLs {
	...
	whitelist := make([]string, 0, len(hosts))
	for host := range hosts {
		whitelist = append(whitelist, host)
	}
	sort.Strings(whitelist)
	// Catalog/parity tests build requests only. Whitelisting their documented
	// vendor endpoints keeps the SSRF guard from doing network DNS lookups.
	secutils.SetSSRFWhitelistFromRaw(strings.Join(whitelist, ","))
