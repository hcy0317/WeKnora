	...
	return &Definition{
		...
		Compat: VendorCompat{
			...
			Rerank: api.RerankCompat{
				// The API reference permits 200 datas, but the pinned
				// vikingdb-go-sdk rejects a request above 50 before sending it.
				// Keep the runtime batch below the verified client ceiling until
				// that SDK-side limit is removed.
				MaxDocuments:   api.Ptr(50),
