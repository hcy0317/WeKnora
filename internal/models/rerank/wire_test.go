...
func probability(doc string) float64 {
	score := float64(utf8.RuneCountInString(doc)) / 100
	if score > 1 {
		return 1
	}
	return score
...
func volcengineDatas(query string, docs []string) []any {
	out := make([]any, len(docs))
	for i, doc := range docs {
		out[i] = map[string]any{"query": query, "content": doc}
	}
	return out
}
...
func TestRerankWireFormatPerVendor(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			wantBody: map[string]any{
				...
				"datas":              volcengineDatas(query, three),
			},
		},
		{
			name:     "volcengine splits at the SDK's 50-document limit",
			provider: "volcengine", model: "doubao-seed-rerank", appSecret: "secret-test",
			docs: documents(51), wantRequests: 2,
			wantPath: "/api/knowledge/service/rerank", wantAuth: [2]string{"Authorization", "Credential=k/"},
			wantBody: map[string]any{
				"rerank_model":       "doubao-seed-rerank",
				"rerank_instruction": volcInstruction,
				"datas":              volcengineDatas(query, documents(50)),
			},
		},
		{
		...
		{
			name:     "lkeap splits at its documented 60-document limit",
			provider: "lkeap", model: "lke-reranker-base", appSecret: "secret-test",
			docs: documents(61), wantRequests: 2,
			wantPath: "/", wantAuth: [2]string{"Authorization", "Credential=k/"},
			wantBody: map[string]any{"Query": query, "Docs": anyStrings(documents(60)), "Model": "lke-reranker-base"},
		},
		{
			name:     "lkeap splits at its documented request-character limit",
			provider: "lkeap", model: "lke-reranker-base", appSecret: "secret-test",
			docs: []string{strings.Repeat("x", 1300), strings.Repeat("y", 1300)}, wantRequests: 2,
			wantPath: "/", wantAuth: [2]string{"Authorization", "Credential=k/"},
			wantBody: map[string]any{"Query": query, "Docs": anyStrings([]string{strings.Repeat("x", 1300)}), "Model": "lke-reranker-base"},
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "volcengine splits at the SDK's 50-document limit" ||
				tc.name == "lkeap splits at its documented 60-document limit" ||
				tc.name == "lkeap splits at its request-character limit" {
				resolved, resolveErr := modelruntime.Resolve(modelruntime.Ref{
					Provider: tc.provider, Model: tc.model, BaseURL: up.url + tc.base,
					ModelType: types.ModelTypeRerank, Extra: tc.extra,
				})
				require.NoError(t, resolveErr)
				expectedLimit := 200
				if tc.provider == "volcengine" {
					expectedLimit = 50
				} else if tc.provider == "lkeap" {
					expectedLimit = 60
				}
				require.Equal(t, expectedLimit, resolved.Rerank.MaxDocuments)
			}
...
func TestRerankerEmptyDocumentsDoesNotCallProvider(t *testing.T) {
	up := newUpstream(t)
	reranker, err := NewReranker(&RerankerConfig{
		Source: types.ModelSourceRemote, Provider: "generic", ModelName: "rerank",
		BaseURL: up.url + "/v1", APIKey: "k",
	})
	require.NoError(t, err)
