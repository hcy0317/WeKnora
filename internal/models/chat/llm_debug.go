...
// truncateForDebug preserves the legacy chat helper name while delegating to
// the protocol-level implementation shared by the new API clients.
func truncateForDebug(value string, maxRunes int) string {
	return api.TruncateForDebug(value, maxRunes)
}
