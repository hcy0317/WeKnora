...
func EffectiveThinkingControl(config *ChatConfig) string {
	...
	switch string(caps.ThinkingFormat) {
	case string(api.ThinkingFormatEnableThinking):
		return "enable_thinking"
	case string(api.ThinkingFormatThinkingType):
		return "thinking_type"
	case string(api.ThinkingFormatChatTemplateKwargs):
		return "chat_template_kwargs"
	default:
		return string(caps.ThinkingFormat)
	}
}
