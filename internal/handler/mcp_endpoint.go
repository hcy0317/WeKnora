...
func mcpEndpointStringValue(p *string) string {
	if p == nil {
		return ""
	}
	return *p
...
func (h *MCPEndpointHandler) CreateMCPEndpoint(c *gin.Context) {
	...
	ep, token, err := h.svc.Create(c.Request.Context(), tenantID, &types.MCPEndpoint{
		Name:               mcpEndpointStringValue(req.Name),
		Description:        mcpEndpointStringValue(req.Description),
		Enabled:            enabled,
		KnowledgeBaseIDs:   types.StringArray(kbIDs),
		Tools:              types.StringArray(tools),
		DefaultAgentID:     mcpEndpointStringValue(req.DefaultAgentID),
