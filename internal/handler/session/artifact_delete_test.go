...
func (s *deleteCatalogStub) Release(_ context.Context, ref, ownerType, ownerID string) (int64, time.Time, error) {
	s.released = append(s.released, ref+"|"+ownerType+"|"+ownerID)
	return s.remaining, time.Time{}, s.releaseErr
}
...
func (s *deleteCatalogStub) ResolvePathForDeletion(ctx context.Context, ref string) (string, *types.StoredResource, error) {
	return s.ResolvePath(ctx, ref)
}
