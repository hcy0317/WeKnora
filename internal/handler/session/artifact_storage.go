...
func (h *Handler) reclaimArtifactBlobs(ctx context.Context, tenantID uint64, refs []types.ArtifactBlobRef) {
	for _, ref := range refs {
		...
		path, ownerTenantID, backendID, claim, ok := h.locateArtifactBlob(ctx, tenantID, ref)
		if !ok {
			continue
		}
		...
		if !claim.IsZero() {
			blobCtx = interfaces.WithResourceDeletionClaim(blobCtx, ref.URL, claim)
		}
...
func (h *Handler) locateArtifactBlob(
...
) (path string, ownerTenantID uint64, backendID string, claim time.Time, ok bool) {
	if h.resourceCatalog == nil {
		...
		return ref.URL, tenantID, "", time.Time{}, true
	}
	...
	for _, messageID := range ref.MessageIDs {
		count, deletionClaim, err := h.resourceCatalog.Release(ctx, ref.URL, types.ResourceOwnerMessage, messageID)
		if err != nil {
	...
	resolved, resource, err := h.resourceCatalog.ResolvePathForDeletion(ctx, ref.URL)
	if err != nil {
		logger.Warnf(ctx, "artifact delete: failed to resolve %s, blob kept: %v", ref.URL, err)
		return "", 0, "", time.Time{}, false
	}
	if resource == nil {
		return resolved, tenantID, "", claim, true
	}
	...
	return resolved, resource.TenantID, resource.StorageBackendID, claim, true
}
