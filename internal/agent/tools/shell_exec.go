...
//
// General shell command execution primitive for the agent. The LLM can inspect files,
...
const legacyShellExecDescription = `Execute a command in the current session's isolated sandbox as the unprivileged sandbox user.
...
- CWD defaults to /workspace on every call; cd does not persist.
  work_dir may select /workspace or the temporary directories /tmp and /var/tmp; missing directories are created as the sandbox user.
...
- For session-only Python extras, run python3 -m pip install --target "$WEKNORA_SKILL_PACKAGE_DIR" <package>.
  Named skill calls add that directory to PYTHONPATH and its node_modules to NODE_PATH.
  The immutable skill image is unchanged; reinstall the skill to make extras permanent.
...
func shellExecDescription(l sandbox.WorkspaceLayout) string {
	if l.IsHost() {
		...
		return fmt.Sprintf(hostShellExecDescription, root, root, root, root)
	}
...
const hostShellExecDescription = "Execute a command in %s. The process is OS-sandboxed on this machine.\n" +
	...
	"- One-off skill packages may be installed under %s/.skill-packages/<skill_name>; " +
	"named calls add that path to PYTHONPATH and NODE_PATH for this workspace.\n" +
...
func (t *ShellExecTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	if hint := t.recoveryHint(input.SkillName, res.ExitCode, command, res.Stdout, stderr); hint != "" {
		b.WriteString(hint)
		b.WriteString("\n")
	}
...
//
// Remote calls keep the local /workspace boundary while allowing temporary
// work under /tmp and /var/tmp. This preserves ordinary scratch workflows
// without exposing system directories as model-selected working roots. A host
// workspace is the user's own disk, so work_dir is clamped to its writable
...
func (t *ShellExecTool) workDirRootsFor(layout sandbox.WorkspaceLayout) []string {
	...
	if !layout.IsHost() {
		return []string{sandbox.SessionWorkspaceRoot, "/tmp", "/var/tmp"}
	}
...
func shellExecRecoveryHint(exitCode int, command, stderr string) string {
	...
	if h := skillOnDemandInstallHint(skillNameFromShellCommand(command), command, "", stderr); h != "" {
		parts = append(parts, h)
...
func (t *ShellExecTool) recoveryHint(skillName string, exitCode int, command, stdout, stderr string) string {
	if exitCode == 0 {
		return ""
	}
	if hint := skillOnDemandInstallHint(skillName, command, stdout, stderr); hint != "" {
		return hint
	}
