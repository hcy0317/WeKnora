...
func TestNormalizeAllowedToolsMapsRetiredRetrievalTools(t *testing.T) {
	...
	want := []string{ToolThinking, LegacyToolKnowledgeSearch, ToolSearchKnowledge, ToolReadDocument, ToolWikiSearch}
...
func TestSuccessorToolName(t *testing.T) {
	cases := map[string]string{
		LegacyToolKnowledgeSearch:     LegacyToolKnowledgeSearch,
	...
	if !IsLegacyRetrievalTool(LegacyToolGrepChunks) ||
		IsLegacyRetrievalTool(LegacyToolKnowledgeSearch) || IsLegacyRetrievalTool(ToolSearchKnowledge) {
...
func TestRetiredRetrievalToolsExplainTheirReplacement(t *testing.T) {
	...
	if RetiredToolReplacement(LegacyToolKnowledgeSearch) != "" {
		t.Fatal("the retained multi-query knowledge_search tool must not be treated as retired")
	}
