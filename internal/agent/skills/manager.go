...
const skillPackageEnvVar = "WEKNORA_SKILL_PACKAGE_DIR"
...
type Manager struct {
	...
	// envResolver is retained for the local service wiring contract. The
	// consolidated shell_exec tool receives the same resolver directly.
...
// WithEnvResolver attaches the per-caller skill environment source used by
// local callers that construct a manager before the consolidated shell tool.
