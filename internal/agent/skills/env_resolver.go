...
// applySessionPackagePath exposes packages installed with --target in this
// session's writable workspace. It precedes caller paths so the package the
// user just installed is visible to the skill interpreter without modifying
// the immutable skill image.
func applySessionPackagePath(env map[string]string, skillName string) {
	if env == nil {
		return
	}
	applySessionPackagePathAt(env, sandbox.SessionSkillPackageDir(skillName))
}

func applySessionPackagePathAt(env map[string]string, dir string) {
	if env == nil || dir == "" {
		return
	}
	prependPathEnv(env, pythonPathEnvVar, dir)
	prependPathEnv(env, nodePathEnvVar, path.Join(dir, "node_modules"))
}
