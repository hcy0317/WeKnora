...
func TestApplySessionPackagePathPrependsPythonAndNodePath(t *testing.T) {
	dir := sandbox.SessionSkillPackageDir("pdf")
	env := map[string]string{
		pythonPathEnvVar: "/caller/python",
		nodePathEnvVar:   "/caller/node",
	}
	applySessionPackagePath(env, "pdf")
	require.Equal(t, dir+":/caller/python", env[pythonPathEnvVar])
	require.Equal(t, dir+":/caller/node", env[nodePathEnvVar])
}
