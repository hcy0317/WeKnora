...
def _find_soffice() -> str | None:
    ...
    try:
        result = subprocess.run(
            ["which", "soffice"], capture_output=True, text=True, check=False
        )
        if result.returncode == 0 and result.stdout.strip():
            return result.stdout.strip()
    except OSError:
...
def _rasterize_with_libreoffice(data: bytes, suffix: str) -> bytes | None:
    """Rasterize vector office media through LibreOffice Draw.

    Debian's ImageMagick build commonly lacks the WMF/EMF delegate, while the
    docreader image already ships LibreOffice for legacy DOC conversion.  Draw
    can decode those same embedded metafiles and export a real PNG without
    ...
    if not soffice:
        return None
    with tempfile.TemporaryDirectory() as temp_dir:
        src = os.path.join(temp_dir, f"input{suffix or '.bin'}")
        ...
        try:
            ...
            logger.warning("LibreOffice image conversion failed: %s", exc)
            return None
        if result.returncode != 0:
            stderr = (result.stderr or b"").decode("utf-8", errors="ignore")
            logger.warning(
                "LibreOffice image conversion exit %s: %s",
                result.returncode,
                stderr,
            )
            return None
        ...
        if not png_candidates:
            ...
            logger.warning("LibreOffice produced no PNG output: %s", stdout)
            return None
        with open(png_candidates[0], "rb") as handle:
            return handle.read()
...
def rasterize_media_bytes(name: str, data: bytes) -> bytes | None:
    ...
    if ext in _RASTER_EXT:
        ...
        if png:
            return png
    png = _rasterize_with_imagemagick(data, ext or ".bin")
    if png:
        return png
    return _rasterize_with_libreoffice(data, ext or ".bin")
